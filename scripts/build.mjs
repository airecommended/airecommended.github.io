...
function renderPage({ page, totalPages, sites, allSites, updatedDate, topic = null }) {
  ...
  const title = `${topic ? topic.label : "全宇宙最全API中转站导航"}：${number.format(allSites.length)} 家站点目录${page > 1 ? ` · 第 ${page} 页` : ''}`;
  const description = `${topic ? "AI API" : "全宇宙最全API"} 中转站导航，收录 ${allSites.length} 家站点，每页展示 1000 家，提供检测成功率、检测次数和站点特色。`;
...
function renderSitemap(totalPages, lastmodDate, topicUrls = []) {
  const urls = [
