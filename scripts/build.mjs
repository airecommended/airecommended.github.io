...
async function syncData() {
  ...
  const capped = { ...incoming, sites: incoming.sites.slice(0, MAX_SITES) };
  ...
  <body>
    ...
    <main id="main">
      ...
      <section class="hero topic-hero">
        <div class="hero__copy"><p class="eyebrow">MODEL DIRECTORY · ${updatedDate.replaceAll("-", ".")}</p><h1>${escapeHtml(topic.label)}<br /><em>推荐与对比</em></h1><p class="hero-copy">${escapeHtml(topic.intro)}</p><p class="update-note">更新于 <time datetime="${updatedDate}">${formatChineseDate(updatedDate)}</time></p><div class="hero-actions"><a href="#topic-ranking">查看候选站</a><a href="#topic-guide">阅读专项指南</a></div></div>
...
function homeGuide() {
  ...
  return `      <section class="decision-guide" id="guide" aria-labelledby="guide-title">
        ...
        <div class="warning-box"><strong>先小额，再加量</strong><p>中转服务可能调价、更换上游或停止运营。第一次只充最低金额，完成多时段测试和账单复算后再增加用量；关键业务保留备用接口，不向不了解的链路提交敏感数据。</p></div>
      </section>
      ...
      <section class="channel-section" aria-labelledby="channel-title">
        ...
        <div class="risk-note"><strong>极低价格需要多做一步成本核验。</strong><p>公开促销、批量采购和高利用率可以合理降价；如果价格长期低到难以覆盖正常成本，且渠道性质、账单和限制都说不清，就应降低预存金额并避免传输敏感数据。不要仅凭低价直接下结论，也不要替无法解释的价格模型承担风险。</p></div>
      </section>
