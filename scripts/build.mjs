...
function renderPage({ page, totalPages, sites, allSites, updatedDate, topic = null }) {
  ...
  const title = `${topic ? topic.label : "API 中转站推荐"}：${number.format(allSites.length)} 家站点目录${page > 1 ? ` · 第 ${page} 页` : ''}`;
  const description = `AI API 中转站目录，收录 ${allSites.length} 家站点，每页展示 1000 家，提供检测成功率、检测次数和站点特色。`;
